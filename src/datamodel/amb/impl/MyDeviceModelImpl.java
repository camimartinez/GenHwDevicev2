...
public class MyDeviceModelImpl extends datamodel.base.impl.DeviceModelImpl implements datamodel.amb.MyDeviceModel {
	...
	public String CreateModel() {
		...
		// Get the Main
		main = ambFac.createMainAMB(spreadsheet[mainIndex][2]);
		...
		for (i = 2; i < spreadsheet[monitorIndex].length; i++) {
			...
			if(!spreadsheet[monitorIndex][i][1].startsWith(table.getDepChar())){
				mp = new MonitorAMBImpl(row, null);
				mparent = mp;
			}
			else{
				mp = new MonitorAMBImpl(row, mparent);
		...
		MonitorAMBImpl[] list = (MonitorAMBImpl[])list2.toArray(new MonitorAMBImpl[0]);
		for (MonitorAMBImpl mp : list) {
			if (mp.isWorldDataArray() && !mp.hasDependents())
				for (i=0; i < Integer.parseInt(mp.NumberItemsWorldData()); i++) {
					...
					MonitorAMBImpl dep = new MonitorAMBImpl(row,mp);
					mp.addDependent(dep);
		...
		for (i = 2; i < spreadsheet[controlIndex].length; ++i) {
			...
			{
				cp = new ControlAMBImpl(row, null);
				cparent = cp;
			}
			else{
				cp = new ControlAMBImpl(row, cparent);
